    ...
    // ---- 负载均衡: 保留 K-means 空间聚类, 仅在失衡时做局部调整 ----
    {
        ...
        else if (std::abs(w0 - w1) > c1.capacity + 1e-9)
        {
            ...
            std::sort(heavy.begin(), heavy.end(),
                      [&](int a, int b) { return pkgs[a].deadline < pkgs[b].deadline; });
            double wh = std::max(w0, w1);
            double wl = std::min(w0, w1);
            double target = (wh - wl) / 2.0;

            vector<int> new_heavy;
            double moved = 0.0;
            for (int pid : heavy)
            {
...
vector<int> Task5::destroy_worst(CarPlan (&cars)[2])
{
    ...
    vector<std::pair<double, int>> scored; // (score, pid)
    for (int c = 0; c < 2; ++c)
        for (const auto& t : cars[c].trip_plans)
            for (int pid : t)
            {
...
vector<int> Task5::destroy_time_window(CarPlan (&cars)[2])
{
    ...
    {
        // Each package belongs to one car plan. Pick the valid delivery time (>= 0).
...
{
    const double cap[2] = {c1.capacity, c2.capacity};
    ...
    while (!pool.empty())
    {
        ...
        for (size_t pi = 0; pi < pool.size(); ++pi)
        {
            ...
            for (int c = 0; c < 2; ++c)
            {
                ...
                for (int ti = 0; ti < n_trips; ++ti)
                {
                    ...
                    if (dest_exists)
                    {
                        double delta = load_bias; // 同目的地, 仅负载惩罚
                        if (delta < best_delta)
                        {
                            best_delta = delta;
        ...
        if (best_pool_idx == -1) { pool.clear(); return; }
